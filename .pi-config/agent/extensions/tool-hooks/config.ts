...
function assertCommand(
  ...
  assertObject(hook, `tool-hooks: ${event}[${groupIndex}].hooks[${hookIndex}] must be an object`);
  ...
  if (hook.timeout !== undefined && (typeof hook.timeout !== "number" || !Number.isFinite(hook.timeout))) {
    throw new Error(`tool-hooks: ${event}[${groupIndex}].hooks[${hookIndex}] timeout must be a finite number`);
  }
}
...
function assertGroup(group: ClaudeHookMatcherGroup, event: ClaudeHookEventName, groupIndex: number) {
  ...
  if (group.matcher !== undefined && typeof group.matcher !== "string") {
    throw new Error(`tool-hooks: ${event}[${groupIndex}].matcher must be a string when provided`);
  }
...
export function validateToolHooksConfig(input: unknown): ClaudeHooksConfigFile {
  ...
  for (const event of Object.keys(hooksValue)) {
    ...
    if (!Array.isArray(hooksValue[event])) {
      throw new Error(`tool-hooks: ${event} must be an array of matcher groups`);
    }
  }
    ...
    groups.forEach((group, groupIndex) => {
      group.hooks.forEach((hook, hookIndex) => {
        normalized.push({
          ...
          timeoutMs: hook.timeout === undefined ? DEFAULT_TIMEOUT_MS : Math.max(1, hook.timeout) * 1000,
        });
      });
    });
  }
