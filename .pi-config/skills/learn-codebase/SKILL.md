...
[ -d ".agents/skills" ] && echo "FOUND: .agents/skills/"
```
...
4. **Skills directories** (`.claude/skills/`, `.cursor/skills/`, `.agents/skills/`) — list available skills and read their descriptions
...
- [List skills found in .claude/skills, .cursor/skills, .agents/skills]
```
...
If `.claude/skills/`, `.agents/skills/`, or other skill directories exist, suggest registering them in `.pi/settings.json` so pi can use them too:
