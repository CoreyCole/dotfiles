...
1. **Paragraph size.** Maximum six sentences per paragraph (Rule 6.6).
1. **Multi-word nouns.** Any noun chain over three words → break it with prepositions (Rule 2.1).
1. **Instructions per sentence.** One, unless the actions are simultaneous (Rule 5.2).
...
1. **Warnings.** Command or condition first, risk second (Rules 7.2, 7.3).
1. **Completeness.** Articles present, "that" present after "make sure", no telegraph style (Rule 4.2).
1. **Untouchables intact.** Code, identifiers, quoted errors, and proper nouns are unchanged.
