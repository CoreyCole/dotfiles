...
1. **Apply the rules** from the catalog below.
1. **Do the self-check** before you deliver. This step is not optional.
1. **Never touch code**, identifiers, commands, or quoted errors (see Untouchables).
...
1. Search your draft for: `'ll`, `'re`, `'s` (contraction), `has been`, `have been`, `should`, `-ing` verbs after a comma, semicolons.
1. Search for every `if` and `when`. Each one stands at the START of its sentence, before the command. "Increase the timeout if the network is slow" → "If the network is slow, increase the timeout."
1. Search for the verbs you did NOT pick in Your Task step 3 (the check/verify/confirm set). Replace every hit with your chosen verb.
...
> 1. If the database is managed (RDS, Cloud SQL), make sure that the instance accepts connections from the IP of sqlpipe.
> 1. If the network is slow, increase `source.connect_timeout_seconds` in the configuration.
