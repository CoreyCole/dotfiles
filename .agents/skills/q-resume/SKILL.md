...
- If `status: in_progress` - continue the current stage from where it left off. You are working on the `[stage]` stage.
  - For `stage: implement`, first apply the repository's submission model from `AGENTS.md`/the handoff. In `cn-agents`, do **not** create branches or run Graphite; stay on `main` in the recorded fresh workspace and use direct `git commit` per completed edit slice. In Graphite repos, create a new stacked Graphite branch only when the first unchecked slice has planned tracked source/test/doc edits.
