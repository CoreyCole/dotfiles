...
2. Inspect `.sql`, migrations, generated query code, repository callers, tests, fixtures, and adjacent query patterns.
3. Run lightweight safe verification when practical (`sqlc generate`, project query generation, targeted tests, static SQL validation). Do not run destructive migrations against shared databases.
