...
function M.exec(client_id, command, smods, range_given, show_vertical, line1, line2)
    ...
    if range_given then
        range =
            vim.lsp.util.make_given_range_params({ line1, 0 }, { line2, math.huge }, 0, client.offset_encoding).range
...
local function make_query_mapping(show_vertical)
    return function(type, client_id)
        ...
        if type == "line" then
            range =
                vim.lsp.util.make_given_range_params({ lnum1, 0 }, { lnum2, math.huge }, 0, client.offset_encoding).range
            range["end"].character = range["end"].character - 1
...
local connection_prompt_function =
    make_prompt_function("showConnections", format_connection_answer, "SqlsConnectionChoice")
