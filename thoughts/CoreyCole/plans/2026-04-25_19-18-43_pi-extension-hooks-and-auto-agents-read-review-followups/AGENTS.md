...
- This follow-up intentionally addresses only the implementation-review P1: `.pi-config/extensions/tool-hooks/index.ts` must load `.pi-config/config/tool-hooks.json` when Pi imports it through `~/.pi/agent/extensions -> ../extensions`.
- Resolve the config from Pi's runtime agent directory (`getAgentDir()` parent), not from `import.meta.url`; the latter can preserve the symlinked `agent/extensions` path and point at missing `~/.pi/agent/config/tool-hooks.json`.
- Canonical facts: `research/2026-04-25_20-06-47_tool-hooks-config-path.md` and `plan.md`.
