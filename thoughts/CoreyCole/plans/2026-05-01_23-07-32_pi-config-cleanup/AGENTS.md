...
- `.pi-config` is intended to be symlinked as `~/.pi`; tracked config and ignored runtime state may coexist in that directory.
- `setup.sh` should not install packages or external dependencies; it should validate and print clear missing-dependency/manual-install guidance.
- Root `.pi-config/package.json` and `package-lock.json` should remain for TypeScript/LSP support for local extensions, not runtime bootstrapping.
...
- Source request: `prds/user-request.md`
- Question-stage inventory: `context/question/lightweight-inventory.md`
