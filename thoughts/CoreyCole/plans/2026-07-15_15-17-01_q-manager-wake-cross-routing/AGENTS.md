...
## Plan-Specific Memory

- **Current focus:** research complete; next stage designs concurrency-safe payload delivery and child lifecycle behavior.
- **Canonical context:** brainstorm/alignment at `context/brainstorms/2026-07-15_15-17-01_q-manager-wake-cross-routing.md`; research agenda at `questions/2026-07-16_17-08-11_q-manager-wake-cross-routing.md`; findings at `research/2026-07-16_17-11-25_q-manager-wake-cross-routing.md`.
- **Scope:** all common `PasteText` traffic—wakes, steering, reprompts, and additional callers found—not wake-only.
- **Invariant:** guarantee payload-to-pane pairing under arbitrary concurrent injections sharing one tmux server.
- **Ambiguity to research:** same checkout/different plan dirs versus exact same canonical plan dir; separate state/lock isolation from transport isolation.
- **Reported companion symptom:** after misrouting, a completed child pane can remain open instead of being closed/replaced by the next child; research must trace this against pane-conflict stopping and pending-cleanup sequencing.
- **Gotcha:** separate state-specific payload construction converges on one fixed tmux-server-global buffer used by separate set/paste commands; deterministic reproduction delivered B's payload into A's correctly targeted pane.
- **Lifecycle finding:** a lost wake leaves A uncontinued; a foreign-state continue from pane A stops at live pane-conflict detection before transition/replacement/cleanup.
- **Split finding:** replacement requests carry `ParentPaneID`, but production `SplitPane` targets the caller's `TMUX_PANE` instead.
- **ADR candidates:** tmux delivery concurrency contract; transport resource identity/lifetime.
