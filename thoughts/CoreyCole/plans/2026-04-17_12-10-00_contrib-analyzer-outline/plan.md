...
## Status
- [x] Slice 1: CLI and config rewrite
- [x] Slice 2: Local git collection primitives
- [x] Slice 3: Local row builder and exporter
- [x] Slice 4: Command integration and docs cleanup
...
Implementation note discovered during execution: `cmd/contrib_export.go` depends on the export package, and the current export package still references the GitHub-era config until Slice 3 lands. After applying the Slice 1 config rewrite, package-level compilation is temporarily broken outside `pkg/contrib/config`. Keep the code changes from Slice 1, but defer the `go run . contrib export --help` verification from this slice to Slice 3 once the exporter has been rewritten.
...
Deferred to Slice 3 after the export package rewrite restores package-level compilation:
