...
- The hook work should target the same functional goal as Claude Code hooks, adapted to Pi's actual extension/runtime surfaces.
- The `AGENTS.md` auto-read behavior should trigger only around `read` tool usage.
- Relevant instruction files are ancestor `AGENTS.md` files from the target file's directory upward.
- Auto-read should be visible in session history, not silent.
