{
  ...
  in {
    ...
    nixosConfigurations.vm-qemu = mkSystem "vm-qemu" rec {
      system = "x86_64-linux";
      user   = "coreycole";
    };

    # Build VM image with: nix build .#vm-image
    vm-image = self.nixosConfigurations.vm-qemu.config.system.build.qcow2;
